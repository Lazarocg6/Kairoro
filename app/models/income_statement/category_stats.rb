      ...
      <<~SQL
        WITH period_totals AS (
          SELECT
            ...
            CASE WHEN t.kind = 'loan_payment' THEN 'expense' WHEN ae.amount < 0 THEN 'income' ELSE 'expense' END as classification,
            SUM(CASE WHEN t.kind = 'loan_payment' THEN ABS(ae.amount * COALESCE(er.rate, 1)) ELSE ae.amount * COALESCE(er.rate, 1) END) as total
          FROM transactions t
          ...
          GROUP BY c.id, period, CASE WHEN t.kind = 'loan_payment' THEN 'expense' WHEN ae.amount < 0 THEN 'income' ELSE 'expense' END
        )
        SELECT
          category_id,
          classification,
