...
class IncomeStatementTest < ActiveSupport::TestCase
  ...
  test "excludes investment_contribution transactions from income statement" do
    # Transferring cash to a brokerage account is not spending — the money
    # still belongs to the user. investment_contribution is in
    # BUDGET_EXCLUDED_KINDS, so it should not affect income or expense totals.
    create_transaction(
    ...
    # investment_contribution should be fully excluded from totals
    assert_equal 4, totals.transactions_count
    assert_equal Money.new(1000, @family.currency), totals.income_money
    assert_equal Money.new(900, @family.currency), totals.expense_money
  end
  ...
  test "excludes provider-imported investment_contribution inflows from income statement" do
    # Simulates a 401k contribution auto-deducted from payroll where the
    # provider imports the contribution as an inflow (negative amount) to the
    # investment account. Like any investment_contribution, this should not
    # appear as either income or expense.
